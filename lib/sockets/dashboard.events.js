...
module.exports = (io, socket) => {
    ...
    // Remote Update
    socket.on('D_system_update', data => {
        console.log(`Emitting Remote Update Signal to all Licenses`);
        io.emit('SS_remote_update');
    })
    
    // Remote Reboot
    socket.on('D_system_reboot', data => {
        console.log(`Emitting Reboot Signal to all Licenses`);
        io.emit('SS_reboot_all');
	})
	
	// Remote Update By License
	socket.on('D_system_update_by_license', data => {
        console.log(`Emitting Remote Update Signal to`, data);
        io.emit('SS_remote_update_by_license', data);
	})
	
	// Remote Update By License
	socket.on('D_pi_restart', data => {
		console.log(`Emitting Restart Signal to`, data);
		io.emit('SS_pi_restart', data);
	})
    
    // Anydesk ID
    socket.on('D_anydesk_id', data => {
